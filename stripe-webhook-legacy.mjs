...
const stripe = Stripe(Netlify.env.get('STRIPE_SECRET_KEY'));
...
const BREVO_LISTS = {
  essai_demarre: Netlify.env.get('BREVO_LIST_ID_ESSAI_DEMARRE'),
  essai_annule: Netlify.env.get('BREVO_LIST_ID_ESSAI_ANNULE'),
  abonnement_annule: Netlify.env.get('BREVO_LIST_ID_ABONNEMENT_ANNULE'),
  paiement_echoue: Netlify.env.get('BREVO_LIST_ID_PAIEMENT_ECHOUE'),
};

async function syncToBrevo(email, listKey, attributes = {}) {
  const listId = BREVO_LISTS[listKey];
  if (!Netlify.env.get('BREVO_API_KEY') || !listId) {
...
export const handler = async (event) => {
  ...
  let stripeEvent;
  try {
    stripeEvent = stripe.webhooks.constructEvent(event.body, sig, Netlify.env.get('STRIPE_WEBHOOK_SECRET'));
  } catch (err) {
