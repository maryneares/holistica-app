...
import {checked,sendOnce,withEmailFooter} from './customer-flow.mjs';
const escapeText=v=>String(v||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export const campaignEmailKey=(id,email)=>'campaign-'+id+'-'+createHash('sha256').update(email.trim().toLowerCase()).digest('hex');
export async function runScheduledCampaigns({db,mailer,render,now=new Date(),budgetMs=18000,clock=Date.now}){
 const started=clock(),results={checked:0,sent:0,partial:0,errors:[]};
 const campaigns=checked(await db.from('crm_campaigns').select('*').eq('status','scheduled').lte('scheduled_at',now.toISOString()))||[];
 results.checked=campaigns.length;
 for(const campaign of campaigns){
  if(clock()-started>=budgetMs)break;
  try{
   const html=withEmailFooter(campaign.blocks?render(campaign.blocks,{headerTag:campaign.header_tag}):campaign.body_html||'');
   let contacts;
   if(campaign.contact_ids?.length)contacts=checked(await db.from('crm_contacts').select('id,email,first_name,blocked').in('id',campaign.contact_ids))||[];
   else contacts=(checked(await db.from('crm_list_contacts').select('crm_contacts(id,email,first_name,blocked)').eq('list_id',campaign.list_id))||[]).map(r=>r.crm_contacts);
   contacts=[...new Map(contacts.filter(c=>c?.email&&!c.blocked).map(c=>[c.email.trim().toLowerCase(),{...c,email:c.email.trim().toLowerCase()}])).values()];
   const receipts=checked(await db.from('holistica_transactional_emails').select('delivery_key,sent_at,resend_id').like('delivery_key','campaign-'+campaign.id+'-%'))||[];
   const done=new Map(receipts.filter(r=>r.sent_at).map(r=>[r.delivery_key,r]));
   const waiting=contacts.filter(c=>!done.has(campaignEmailKey(campaign.id,c.email)));
   for(let offset=0;offset<waiting.length&&clock()-started<budgetMs;offset+=5){
    const outcomes=await Promise.allSettled(waiting.slice(offset,offset+5).map(async c=>{
     const key=campaignEmailKey(campaign.id,c.email);
     await sendOnce(mailer,key,{to:[c.email],subject:campaign.subject,html:html.replace(/\{\{prenom\}\}/g,escapeText(c.first_name)).replace(/\{\{unsubscribe\}\}/g,'https://app.holisticaclub.com/.netlify/functions/crm-unsubscribe?id='+encodeURIComponent(c.id))});
     done.set(key,{delivery_key:key});
    }));
    for(const outcome of outcomes)if(outcome.status==='rejected')results.errors.push('Campagne '+campaign.id+': '+outcome.reason.message);
   }
   // Tracking can be retried independently after provider acceptance, without resending.
   const finalReceipts=checked(await db.from('holistica_transactional_emails').select('delivery_key,sent_at,resend_id').like('delivery_key','campaign-'+campaign.id+'-%'))||[];
   const accepted=new Map(finalReceipts.filter(r=>r.sent_at&&r.resend_id).map(r=>[r.delivery_key,r]));
   const existing=checked(await db.from('crm_campaign_recipients').select('email').eq('campaign_id',campaign.id))||[];
   const tracked=new Set(existing.map(r=>r.email.toLowerCase()));
   const missing=contacts.filter(c=>accepted.has(campaignEmailKey(campaign.id,c.email))&&!tracked.has(c.email));
   if(missing.length)checked(await db.from('crm_campaign_recipients').insert(missing.map(c=>({campaign_id:campaign.id,contact_id:c.id,email:c.email,resend_email_id:accepted.get(campaignEmailKey(campaign.id,c.email)).resend_id,status:'sent'}))));
   const sentCount=contacts.filter(c=>accepted.has(campaignEmailKey(campaign.id,c.email))).length;
   if(sentCount===contacts.length){
    checked(await db.from('crm_campaigns').update({status:'sent',sent_at:now.toISOString(),recipients_count:sentCount,body_html:html}).eq('id',campaign.id).eq('status','scheduled'));
    results.sent++;
   }else results.partial++;
  }catch(error){results.errors.push('Campagne '+campaign.id+': '+error.message);}
 }
 return results;
}
...
export function blocksToEmailHtml(blocks, meta) {
  meta = meta || {};
